...
private struct SurahHeader: View {
    ...
    var body: some View {
        ...
        .padding(.vertical, 20)
        .background(.salahBackground, in: .rect(cornerRadius: 18))
    }
}
        ...
        VStack(alignment: .leading, spacing: 12) {
            HStack {
                Text("\(verse.surah):\(verse.number)")
                    ...
                    .background(Color.salahGreen.opacity(0.12), in: .capsule)
                    .foregroundStyle(Color.salahGreen)
                if isBookmarked {
                    Image(systemName: "bookmark.fill").foregroundStyle(Color.salahGold)
                }
                Spacer()
            }
