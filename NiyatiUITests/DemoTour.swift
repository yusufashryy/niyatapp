...
final class DemoTour: XCTestCase {
    ...
    func test2Tour() {
        ...
        XCTAssertTrue(app.staticTexts["Al-Faatiha"].waitForExistence(timeout: 10))
        pause()
        snapshot("05 Quran - surahs")
        app.staticTexts["Al-Faatiha"].tap()
        pause(2)
        ...
        pause()
        snapshot("10 More")
        app.buttons["Tasbih counter"].tap()
        pause()
